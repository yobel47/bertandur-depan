...
import { deleteTutorialsDetail, loadTutorialsDetail } from "../../redux/actions";
...
const TABLE_HEAD = [
  ...
  { id: "video", label: "URL Video", alignRight: false },
  { id: "" },
];
...
export default function DetailTutorial() {
  ...
  const { status, success, details, error } = useSelector((state) => state.tutorial);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - details.length) : 0;

  const filteredTutorials = applySortFilter(details, getComparator(order, orderBy), filterName);
  ...
  return (
    <>
      <Helmet>
        <title> Detail Tutorial | Tandur Admin </title>
      </Helmet>
      {error?.message == "" ? (
        <ErrorSomethingWrong />
      ) : (
        <>
          <Container>
            ...
            <Card>
              ...
              <TableContainer>
                <Table>
                  <UserListHead order={order} orderBy={orderBy} headLabel={TABLE_HEAD} rowCount={details.length} onRequestSort={handleRequestSort} />
                  <TableBody>
                    ...
                    {filteredTutorials.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                      return (
                        <TableRow hover key={row.ID_TUTORIAL} tabIndex={-1} role="checkbox">
                          ...
                          <TableCell align="left">{row.DESC_TD}</TableCell>
                          <TableCell align="left">{row.URLVIDEO_TD}</TableCell>
          ...
          <DeleteModal
            ...
            onDelete={() => dispatch(deleteTutorialsDetail({ id_td: selectedId, id_tutorial: values.ID_TUTORIAL }))}
            styles={"z-[1300]"}
            title={"tutorial"}
          />
