...
import { loadLandTransactionAdmin, verifLandTransaction } from "../../../../redux/actions";
...
const TABLE_HEAD = [
  ...
  { id: "name", label: "Penyewa", alignRight: false },
  { id: "land", label: "Lahan", alignRight: false },
  { id: "duration", label: "Durasi Sewa", alignRight: false },
...
export default function UrbanFarming() {
  ...
  const { status, success, transaction, error } = useSelector((state) => state.land);

  useEffect(() => {
    dispatch(loadLandTransactionAdmin());
  }, [dispatch]);
    ...
    <>
            ...
            <Table>
              ...
              <TableBody>
                {filteredTransaction.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                  return (
                    <TableRow hover key={row.ID_RENT} tabIndex={-1} role="checkbox">
                      ...
                      <TableCell align="left">{row.NAME_LAND}</TableCell>
                      <TableCell align="left">{row.DURATION_RENT} Bulan</TableCell>
                      <TableCell align="left">{currencyFormat(row.TOTPAYMENT_RENT.toString(), "Rp. ")}</TableCell>

                      <TableCell align="center" width={20}>
                        {row.STATUS_PAYRENT !== 0 && (
      ...
      <ConfirmModal
        ...
        onVerif={() => {
          ...
          formdata.append("id_rent", selectedId);
          dispatch(verifLandTransaction(formdata));
        }}
        styles={"z-[1300]"}
        title={"transaksi"}
      />
